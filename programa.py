    ...
    while not jogada_feita:
        ...
        if opcao == '1':
            ...
            while not combinacao_valida:
                if cat not in todas_combinacoes:
                    print("Combinação inválida. Tente novamente.")
                    cat = input()
                elif cat in combinacoes_simples and cartela['regra_simples'][int(cat)] != -1:
                    print("Essa combinação já foi utilizada.")
                    cat = input()
                elif cat in combinacoes_avancadas and cartela['regra_avancada'][cat] != -1:
                    print("Essa combinação já foi utilizada.")
                    cat = input()
                else:
            ...
            cartela      = faz_jogada(todos_dados, cat, cartela)
            jogada_feita = True
            rodada      += 1
            continue
...
for cat in cartela['regra_avancada']:
    if cartela['regra_avancada'][cat] != -1:
        pontuacao += cartela['regra_avancada'][cat]
